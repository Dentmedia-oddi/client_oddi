function textToCopy(datosPaciente, evaluacionPaciente, dolorPaciente, estudiosPaciente, vitalidadPaciente, diagnosticoPaciente, conductosPaciente, medicacionPaciente, observacionesPaciente) {
  ...
  const restauracionInfo = Object.keys(diagnosticoPaciente).length !== 0
    ...
    : "";
  const anesteciaInfo = Object.keys(diagnosticoPaciente).length !== 0
  ...
  return `${pacienteInfo}${evaluacionInfo}${dolorInfo}${estudiosInfo}${vitalidadInfo}${diagnosticoInfo}${conducto1}${conducto2}${conducto3}${conducto4}${conducto5}${conducto6}${tecnicaObturacion}${restauracionInfo}${anesteciaInfo}${medicacionInfo}${observacionesInfo}`.replace(/ +(?= )/g, "").trim();
}
